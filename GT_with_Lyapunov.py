...
from _utils import _pathloss_, _compute_interference_, _move_avg_, _scale_data_,\
                   obs_list_to_state_vector, unit_map, gen_init_power,\
                    _composite_channel_, dBm2Watt, find_state_percentile, Utility_function, Utility_function_log
from main_WMMSE_OneShot import WMMSE
from f_PlotRewardDist import plot_rwd_hist


def main_GT_w_Lyapunov(SmallNet=True, MSR=20, beamwidth=np.pi/4, n_frames=1000, n_slots=1000, n_trials=3,
                       test_channel=0, alpha=1.0, beta=0.0, power_max_dBm=39, bandwidth=500e6, UE_sched=0,
                       pathloss=None, window_size=200, V=3000, T_f=0.5, T_b=0.5,
                       T_unit=1e-3, W=1, P_avg=1, g_max=1e12):
    r"""
    OUTPUT:
    ...
    for trial_idx in range(n_trials):
        ...
        for frame_idx in range(n_frames):
            ...
            for slot_idx in range(n_slots):
                # print('1: ', frame_idx, slot_idx, power_profile_frame)
                ...
                """Get instant power according to GT"""
                for bs in range(n_BS):
                    "Game Theory based choice"
                    ue_sch = UE_sched[bs]
                    ...
                    a = (H_ji[bs] * W) / max(Z_i[bs], threshold)
                    b = (Interf_profile_frame[bs, slot_idx] + (noise * W)) / \
                        (antenna_gain_max_BS * antenna_gain_max_UE * Direct_channel[frame_idx*n_slots + slot_idx-1, ue_sch, bs] *
                         _pathloss_(BS_UE_distance[ue_g][bs], pathloss))
                    # print(frame_idx, slot_idx, a, b)
                    if slot_idx + 1 < n_slots:
                        power_profile_frame[bs, slot_idx + 1] = min(P_MAX, max(a - b, 0))
                    else:
                        power_profile_frame[bs, slot_idx] = min(P_MAX, max(a - b, 0))
                # print(slot_idx, 'Power: ', power_profile_frame[:, slot_idx])
        ...
        # print(trial_idx, U_trail[trial_idx])
    ...
    Rwd_avg_BS = U_trail.mean(axis=1)  # avg over BSs,size=(num_rep,n_frames)
    print(Rwd_avg_BS)
...
def main(IsSmallNet=True, n_slots=100, n_frames=10000, n_trials=1, window_size=150, bandwidth=500e6):
     ...
     Data_GT, Rwd_GT, Power_GT, Power_avg, H_ji = main_GT_w_Lyapunov(SmallNet=True, MSR=20, beamwidth=beamwidth,
                        n_frames=n_frames, n_slots=n_slots, n_trials=n_trials, test_channel=0, power_max_dBm=p_max_dBm,
                        bandwidth=bandwidth, UE_sched=0, pathloss=pathloss, window_size=window_size,
                        V=V, T_f=T_f, T_b=T_b, T_unit=T_unit, W=W, P_avg=P_avg, g_max=g_max)

     time_finished = time.strftime("%H:%M:%S", time.localtime())
     np.save('npy_data/GT_data_{}.npy'.format(time_finished), Data_GT)

     MSR, BW, n_BS = params["MSR"], params["BW"], len(Power_GT[:, 0])
     label = "GT throughput."

     plt.plot(Data_GT[0], label=label, lw=0.8)
     plt.ylabel('Average throughput GT (bits/sec/Hz per BS)')
     plt.xlabel('Frames')
     # plt.ylim([min(Data_MADDPG[2]) - 0.5, max(Data_MADDPG[1]) + 0.5])
     plt.ylim([0, max(Data_GT[1]) + 0.1])
     ...
     fig.suptitle(f"GT power over time")
     for bs in range(n_BS):
         ax[bs].plot(Power_GT[bs, -int(n_frames / 1):-1], label=f"BS {bs}", lw=0.8)
     ...
     fig, ax = plt.subplots(n_BS, 1, sharex=True)  # plot powers
     fig.suptitle(f"GT H_ji over time")
     for bs in range(n_BS):
     ...
     fig.suptitle(f"GT P_avg over time")
     for bs in range(n_BS):
